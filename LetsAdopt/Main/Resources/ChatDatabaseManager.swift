...
extension ChatDatabaseManager {
    ...
    public func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            let dateString = ChatFrameViewController.dateFormatter.string(from: messageDate)
        ...
        database.child("\(id)/messages").observe(.value, with: { (snapshot) in
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                guard let name = dictionary["name"] as? String,
                    ...
                    let date = ChatFrameViewController.dateFormatter.date(from: dateString)else {
                        return nil
                }
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    if var currentUserConversations = snapshot.value as? [[String: Any]] {
                        ...
                        if var targetConversation = targetConversation {
                            ...
                            let newConversationData: [String: Any] = [
                                "id": conversation,
                                "other_user_email": ChatDatabaseManager.safeEmail(emailAddress: otherUserEmail),
                                "name": name,
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]] {
                                ...
                                if var targetConversation = targetConversation {
                                    ...
                                    let newConversationData: [String: Any] = [
                                        "id": conversation,
                                        "other_user_email": ChatDatabaseManager.safeEmail(emailAddress: currentEmail),
                                        "name": currentName,
                                        "latest_message": updatedValue
                                    ]
